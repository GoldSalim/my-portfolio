...
export default function AboutPage() {
  ...
  const handleNavigation = (path: string): void => {
    if (path) {
      ...
      // Just close the sidebar for "LET'S TALK"
      setIsOpen(false);
    }
  };
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.main 
        ...
        animate="animate"
        exit="exit"
        className="w-full bg-[#EBEAE5] text-[#121ABC] overflow-x-hidden font-sans"
      >
          ...
          <div className="relative w-full flex flex-col h-full">
            ...
            <nav className="flex flex-col mt-28 gap-0 flex-grow">
              {navItems.map((item, idx) => {
                // If the item has a path, render as clickable with navigation
                if (item.path) {
                  return (
                    <div 
                      key={item.name}
                  ...
                  // "LET'S TALK" - no navigation, just closes the sidebar
                  return (
                    <div 
                      key={item.name}
                      onClick={() => setIsOpen(false)}
